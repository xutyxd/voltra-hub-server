...
sed -e "s/ToReplace/$class_name/g" \
    ...
    -e  "s/toReplace/$instance_name/g" << EOF > "$entity_folder/interfaces/data/$entity_name-api-data.interface.ts"
import { IEntityAPIData } from "../../../crosscutting/common/interfaces/data";

export interface IToReplaceAPIData extends IEntityAPIData {
    propertyA: string;
}
EOF